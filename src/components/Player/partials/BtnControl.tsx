...
function BtnControl({ icon, addClass, onClick, title }: Props) {
  return (
    <div className='relative'>
      ...
      {
        title !== undefined ?
          <span className='absolute -top-9 left-1/2 -translate-x-1/2 text-xs px-2 py-1 w-fit whitespace-nowrap bg-gray-300 rounded-md z-30 hidden peer-hover:block text-black'>
            {title}
